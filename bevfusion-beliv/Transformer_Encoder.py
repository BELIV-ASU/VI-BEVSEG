...
class MultiheadAttention(nn.Module):
    ...
    def __init__(self, input_dim, embed_dim, num_heads, cross_label, bev_cross_label):
        ...
        self.patch_in = PatchEmbedding(in_channels=3, patch_size=self.patch_size, embed_dim=embed_dim)
        self.patch_pe = PatchEmbedding(in_channels=256, patch_size=self.patch_size, embed_dim=embed_dim)
    ...
    def forward(self, x, feat_tag=0, mask=None, return_attention=False, pe=None, mask_map=None):
        ...
        conv_layer = nn.AvgPool2d(kernel_size=(self.patch_size,self.patch_size), stride=(self.patch_size,self.patch_size), padding=(0,0))
        patch_map = conv_layer(extend_map)
        patch_map = patch_map.squeeze(0).squeeze(0)
        
        patch_mask = patch_map > 0.0
        batch_end = time.time()
        ...
        if feat_tag == 0:  # initial the concated K,V
            for i in range(len(x)):
                ...
                patch_x = patch_x.view(1,int(256/self.patch_size),int(704/self.patch_size),-1)
        ...
        patch_x = patch_x.reshape(1,int(256/self.patch_size),int(704/self.patch_size),-1)
        ...
        patch_pe = patch_pe.view(1,int(256/self.patch_size),int(704/self.patch_size),-1)
        flatten_pe = patch_pe[:,patch_mask,:].reshape(batch_size,seq_length, self.num_heads, self.head_dim).permute(0, 2, 1, 3)
        end = time.time()
        #print('patch operation takes', end - start)
        ...
        #print('dot operations take:',end - start)
        if return_attention:
            return o, attention
        else:
            return o
...
class EncoderBlock(nn.Module):
    ...
    def __init__(self, input_dim, num_heads, dim_feedforward, cross_label=False, bev_cross_label=False, dropout=0.0):
        ...
        self.patch_size = 8
        self.patch = PatchEmbedding(in_channels=3, patch_size=self.patch_size, embed_dim=input_dim)
    ...
    def forward(self, x, feat_tag=0, mask=None, pe=None, mask_map=None):
        ...
        end_time = time.time()
        #print('attention calculation takes:',end_time - start_time)
