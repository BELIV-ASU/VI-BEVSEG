    ...
    def get_cam_feats(self, x, x_raw):
        ...
        if self.training == True:
            ...
            for i in range(len(x_raw)):
                ...
                d_list.append(per_d)
            end_time = time.time()
            #print('depth model estimation takes:',end_time - start_time)

            
            d = torch.cat([t for t in d_list], dim=0).cuda()   # The size is [6,1,256,704], Ground truth depth
            ...
            #start_time = time.time()
            d_est = self.pyramid_depth_net(x_raw)['out']  # Do depth estimation on original image, input size is [6,3,256,704] [B * N, C, H, W]
            d_est = self.conv(d_est)
            loss = 0
            #end_time = time.time()
            #print('Inference time for depth estimation of one image', end_time - start_time)
