    ...
    def __init__(
        ...
        **kwargs,
        ...
        if fuser is not None:
            ...
            #self.input_cross_transformer = TransformerEncoder(num_layers=3, input_dim=768,dim_feedforward=768*3, num_heads=2,cross_label=True,bev_cross_label=False)
        ...
        **kwargs,
        ...
        for sensor in (
            ...
            if sensor == "camera":
                ...
                img_veh = img[:,0].unsqueeze(1)
                
                camera2ego_veh = camera2ego[:,0].unsqueeze(1)
                lidar2camera_veh = lidar2camera[:,0].unsqueeze(1)
                lidar2image_veh = lidar2image[:,0].unsqueeze(1)
                camera_intrinsics_veh = camera_intrinsics[:,0].unsqueeze(1)
                camera2lidar_veh = camera2lidar[:,0].unsqueeze(1)
                img_aug_matrix_veh = img_aug_matrix[:,0].unsqueeze(1)
                ...
                img_infra = img[:,7].unsqueeze(1)
                camera2ego_infra = camera2ego[:,7].unsqueeze(1)
                lidar2camera_infra = lidar2camera[:,7].unsqueeze(1)
                lidar2image_infra = lidar2image[:,7].unsqueeze(1)
                camera_intrinsics_infra = camera_intrinsics[:,7].unsqueeze(1)
                camera2lidar_infra = camera2lidar[:,7].unsqueeze(1)
                img_aug_matrix_infra = img_aug_matrix[:,7].unsqueeze(1)
        ...
        #end_time = time.time()
        #print('One epoch take',end_time - start_time) 
        if self.training:
            outputs = {}
