...
class BEVFusion(Base3DFusionModel):
    ...
    def extract_camera_features(
        ...
        end_time = time.time()
        #print('Vehicle Feature extraction takes time:', end_time - start_time)
    ...
    def extract_infra_features(
        ...
        x = x.view(B, int(BN / B), C, H, W)
        end_time = time.time()
        #print('Infra Feature Extraction takes time:',end_time - start_time)
        ...
        end_time = time.time()
        #print('Infra Vtransform takes time:', end_time - start_time)
        return x
        ...
        if self.training:
            ...
            for type, head in self.heads.items():
                if type == "object":
                    ...
                    #plt.imshow(gt_masks_bev[0,0,:,:].detach().cpu().numpy())
                    #plt.axis('on')
                    #plt.show()
