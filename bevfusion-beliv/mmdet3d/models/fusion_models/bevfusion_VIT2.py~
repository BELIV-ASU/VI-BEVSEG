...
class BEVFusion(Base3DFusionModel):
    def __init__(
        ...
        **kwargs,
        ...
        if fuser is not None:
            ...
            self.input_cross_transformer = TransformerEncoder(num_layers=3, input_dim=768,dim_feedforward=768*3, num_heads=8,cross_label=True,bev_cross_label=False)
    ...
    def extract_camera_features(
        ...
        #if self.count % 100 == 0:
            #plt.imshow(final_features[0,:,:,:].permute(1,2,0).detach().cpu().numpy())
            #plt.axis('off')
            #plt.show()
    ...
    def extract_infra_features(
        ...
        # Sample edge pixels of the image, crop the [H/2, W/2] center area
        #x_start = int(H/8)
        #x_end = int(7*H/8)
        #y_start = int(W/8)
        #y_end = int(7*W/8)
        ...
        final_features = torch.cat(transformer_features,dim=0)
        #plt.imshow(final_features[0,0,:,:].detach().cpu().numpy())
        #plt.axis('off')
        #plt.show()
