...
def load_h5_file(file_path):
    with h5py.File(file_path, 'r') as f:
        # Create a dictionary to hold the data for each file
        data = {}
        ...
        for name in f:
            if isinstance(f[name], h5py.Dataset):
                ...
                for sub_name in f[name]:
                    data[f"{name}/{sub_name}"] = np.array(f[name][sub_name])
        return data

def load_multiple_h5_files(file_paths):
    all_data = {}
    for file_path in file_paths:
        file_name = os.path.basename(file_path)
        town_name = os.path.splitext(file_name)[0]
        #print(f"Loading {file_path}...")
        all_data[town_name] = load_h5_file(file_path)
    return all_data
def extract_patch(data, x, y, h, w, rotation, town_name, layer_name):
    """
    Extracts a patch from the specified layer and town with the given parameters.

    :param data: Dictionary containing the map data for different towns and layers.
    ...
    :param rotation: Patch orientation in degrees. North-facing corresponds to 0.
    :param town_name: Name of the town (e.g., 'Town03').
    :param layer_name: Name of the layer (e.g., 'drivable_area').
    :return: A numpy array representing the patch.
    """
    layer_key = f"{town_name}/{layer_name}"
    if town_name not in data or layer_key not in data[town_name]:
        #print(f"Layer {layer_name} not found for town {town_name}. Creating an empty layer.")
        height, width = h, w
        return np.zeros((height, width), dtype=np.uint8)
    
    layer_data = data[town_name][layer_key]
    height, width = layer_data.shape

    patch = np.zeros((h, w), dtype=layer_data.dtype)
    ...
    for i in range(h):
        for j in range(w):
            patch_x = x + (j - w // 2) * cos_angle - (i - h // 2) * sin_angle
            patch_y = y + (j - w // 2) * sin_angle + (i - h // 2) * cos_angle
            
            if 0 <= patch_x < width and 0 <= patch_y < height:
                patch[i, j] = layer_data[int(patch_y), int(patch_x)]

    return patch

def generate_tensor(data, town_number, x, y, h, w, rotation):
    """
    Generates a 7 x h x w tensor for the specified town and patch parameters.
    
    :param data: Dictionary containing the map data for different towns and layers.
    :param town_number: Integer corresponding to the town number (3, 4, or 5).
    :param x: X-coordinate of the center of the patch.
    :param y: Y-coordinate of the center of the patch.
    :param h: Height of the patch.
    :param w: Width of the patch.
    :param rotation: Patch orientation in degrees. North-facing corresponds to 0.
    :return: A 7 x h x w numpy array representing the tensor.
    """
    town_name = f"Town0{town_number}"

    mappings = {
        "drivable_area": ["drivable_area"],
        "ped_crossing": ["crosswalk_area"],
        "walkway": ["sidewalk_area"],
        #"stop_line": ["lane_markings"],
        #"carpark_area": ["parking_area"],
        #"divider": ["road_divider", "lane_divider"]
    }
    # if long-map, *2; if short-map, *4
    x1 = x *2.0
    y1 = y *2.0
    ...
    #self_classes = ["drivable_area", "ped_crossing", "walkway", "stop_line", "carpark_area", "divider"]
    self_classes = ["drivable_area", "ped_crossing", "walkway"]

    tensor = np.zeros((len(self_classes), h, w), dtype=np.uint8)

    for k, name in enumerate(self_classes):
        layer_names = mappings[name]
        combined_patch = np.zeros((h, w), dtype=np.uint8)
        for layer_name in layer_names:
            patch = extract_patch(data, x, y, h, w, rotation, town_name, layer_name)
            combined_patch = np.logical_or(combined_patch, patch).astype(np.uint8)
        tensor[k] = combined_patch
...
class LoadBEVSegmentation:
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        file_paths = ['/scratch/jmeng18/V2X-SIM/Town03.h5', '/scratch/jmeng18/V2X-SIM/Town04.h5', '/scratch/jmeng18/V2X-SIM/Town05.h5']
        all_data = load_multiple_h5_files(file_paths)
        all_data['Town03']['Town03/drivable_area'] = np.logical_or(all_data['Town03']['Town03/drivable_area'], all_data['Town03']['Town03/parking_area']).astype(int)
        all_data['Town04']['Town04/drivable_area'] = np.logical_or(all_data['Town04']['Town04/drivable_area'], all_data['Town04']['Town04/parking_area']).astype(int)
        all_data['Town05']['Town05/drivable_area'] = np.logical_or(all_data['Town05']['Town05/drivable_area'], all_data['Town05']['Town05/parking_area']).astype(int)
        town_number = location
        x, y, h, w, rot = map_pose[0], map_pose[1], 200, 200, patch_angle
        labels = generate_tensor(all_data, town_number, x, y, h, w, rot)
